#!/usr/bin/env python3
"""
Extract all images from PwC content element slides and save them.
This will extract the actual photos, logos, and graphics from each element.
"""
    ...
    for shape in slide.shapes:
        ...
        if shape.shape_type == MSO_SHAPE_TYPE.PICTURE:
            ...
            try:
                ...
                # Save the image
                image_filename = f"element_{slide_num:02d}_image_{image_count}.{ext}"
...
def update_element_json_with_images(element_json_path, images_info, images_relative_path):
    """Update the element JSON file to reference the actual extracted images"""
    
    with open(element_json_path, 'r', encoding='utf-8') as f:
        element_data = json.load(f)
    
    # Update image components with actual image paths
    components = element_data['slide']['components']
    image_index = 0
...
def main():
    ...
    elements_dir = Path("/Users/ahmshalan/Projects/mobifly/ppt-generate/branding/content_elements")
    images_dir = elements_dir / "images"
    json_dir = elements_dir / "elements"
    ...
    print(f"Loading presentation from: {elements_file}")
    ...
    for idx, slide in enumerate(prs.slides, start=1):
        ...
        try:
            ...
            if images_info:
                ...
                # Find the corresponding JSON file (try all possible categories)
                json_files = list(json_dir.glob(f"element_{idx:02d}_*.json"))
                if json_files:
                    json_file = json_files[0]
                    update_element_json_with_images(json_file, images_info, "images")
                    print(f"    ✓ Updated JSON: {json_file.name}")
                else:
                    print(f"    ⚠ No JSON file found for element {idx}")
            else:
    ...
    # Create image index
    image_index = {
        "total_elements": total_slides,
        "elements_with_images": len(all_extractions),
    ...
    print(f"  Total elements: {total_slides}")
    print(f"  Elements with images: {len(all_extractions)}")
